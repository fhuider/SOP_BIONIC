...
<pre>
# First time running Steps 4.1 - 4.10:
tar -c *.log -zv -f log_files.tar.gz
tar -c *.dat -zv -f dat_files.tar.gz
tar -c *_CLN.* -zv -f cln_geno_files.tar.gz

# Second, third, or xth run, add a corresponding suffix to the tar output as to not overwrite your earlier archives:
# tar -c *.log -zv -f log_files.tar_[<i>x</i>].gz
# tar -c *.dat -zv -f dat_files.tar_[<i>x</i>].gz
# tar -c *_CLN.* -zv -f cln_geno_files_[<i>x</i>].tar.gz
